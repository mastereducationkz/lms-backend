...
from src.schemas.models import (
    UserInDB, Group, GroupStudent, Course, Module, Lesson, CourseGroupAccess,
)
...
def _grant_course_access(db, group, course, granted_by_user_id):
    """check_course_access() for students only checks CourseGroupAccess (not Enrollment),
    so tests calling mark_lesson_complete must wire this up or the call 403s."""
    db.add(CourseGroupAccess(course_id=course.id, group_id=group.id,
                             granted_by=granted_by_user_id, is_active=True))
    db.flush()
...
def test_completing_last_unit_reports_newly_ready(db):
    student = _student(db)
    ...
    # l1 not yet completed; mark_lesson_complete should complete it and report a as ready
