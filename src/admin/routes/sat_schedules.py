...
def _get_date_range(filter_type: FilterType):
    """Return (start, end) UTC-naive datetime bounds for a given filter, or (None, None) for 'all'."""
...
async def get_sat_group_schedules(
    filter: FilterType = Query("all", description="Filter: all | past | upcoming | today | this_week | this_month"),
    ...
    Authentication: X-API-Key header (MASTEREDU_API_KEY).

    Supported filter values:
    - all        — every event regardless of date
    - past       — events that have already started (start_datetime < now)
    - upcoming   — events that haven't started yet (start_datetime > now)
    - today      — events occurring today
    - this_week  — events in the current calendar week (Mon–Sun)
    - this_month — events in the current calendar month
    """
    date_start, date_end = _get_date_range(filter)
