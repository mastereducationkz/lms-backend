...
def _create_exam_collection_task_if_needed(
    ...
    *,
    ...
    trigger_due = _build_exam_due_date(trigger_date)
    # Show task starting from the results week (Monday of trigger week),
    # not only on the exact trigger date.
    trigger_week_start = trigger_date - timedelta(days=trigger_date.weekday())
    if today < trigger_week_start:
        return 0
    ...
    if exists:
        # Keep existing dedupe, but heal legacy tasks that were created without due_date
        # or with outdated scheduling metadata.
        should_update = False
        if exists.due_date != trigger_due:
            exists.due_date = trigger_due
            should_update = True
        if exists.curator_id != curator_id:
            exists.curator_id = curator_id
            should_update = True
...
def generate_exam_result_collection_tasks(db, reference_date: date_type | None = None) -> int:
    ...
    if created_count > 0 or has_backfill_updates:
        ...
        logger.info(f"[SCHEDULER] Exam-result run complete: created={created_count} backfill_updates={has_backfill_updates}")
    else:
        db.rollback()
    ...
    for group in groups:
        ...
        for tmpl in templates:
            if tmpl.task_type == "manual":
                continue
            # exam_results_collection is generated by dedicated logic
            # (generate_exam_result_collection_tasks), not weekly templates.
